...
export async function GET(req: NextRequest) {
  ...
  const stats = db.prepare(`
    SELECT
      ...
      COALESCE(SUM(CASE WHEN payment_status = 'PAID' OR status = 'PAID' THEN COALESCE(total_amount, amount, 0) ELSE 0 END), 0) as totalDisbursed,
      COALESCE(SUM(CASE WHEN payment_status IN ('PENDING', 'PROCESSING', 'ON_HOLD') OR status IN ('PENDING', 'PROCESSING') THEN COALESCE(total_amount, amount, 0) ELSE 0 END), 0) as pendingDisbursement,
